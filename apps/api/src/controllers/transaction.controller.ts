...
export class TransactionController {
  ...
  // ✅ ЭТОТ МЕТОД УЖЕ ПРАВИЛЬНЫЙ! Он использует TransactionService
  ...
  static async getByDateRange(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!start || !end) {
        return res.status(400).json({ success: false, message: 'Параметры start и end обязательны' });
      }
  ...
  static async getDailySummary(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      if (!date) {
        return res.status(400).json({ success: false, message: 'Параметр date обязателен' });
      }
